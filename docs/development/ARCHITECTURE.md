...
9. The selected account remains bound to that upstream WebSocket while it is healthy. Later `response.create` frames re-evaluate policy plus quota/cooldown and circuit-breaker availability on the bound account; a transient availability rejection replaces the upstream connection without closing the persistent local client when the frame is replayable.
10. An upstream close between requests leaves the local WebSocket open. A close during a request is replayed on a replacement upstream only when no upstream response event was forwarded and the frame has no connection-scoped `previous_response_id`, preventing duplicate partial output and invalid continuation reuse. If a replacement upstream would receive a connection-scoped continuation, the proxy emits the Responses protocol's retryable `previous_response_not_found` error so the client discards cached socket state and retries with full context. Exhausted transient recovery waits with exponential backoff capped at 60 seconds for replayable frames.
11. WebSocket terminal events settle usage in request order and update rate-limit, auth, server-error, cooldown, retry, and affinity state for later upstream selections; explicit token invalidation during refresh or handshake stops account failover.
12. Successful HTTP/SSE responses stream back to the local Codex client with hop-by-hop/private/stale decoded headers removed, while WebSocket frames remain bidirectional until the local client closes or an already-visible partial response makes transparent replay unsafe.
13. Usage ledger rows and runtime counters are persisted for status/report/usage/budget commands.
14. On exit, shadow-home branches sync refreshed official state files back and remove the temporary directory. The canonical-home branches have nothing to sync because they read and wrote official state in place.
