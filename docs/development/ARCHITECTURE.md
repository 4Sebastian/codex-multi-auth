...
9. The selected account remains bound to that upstream WebSocket while it is healthy. Later `response.create` frames re-evaluate policy and consume request tokens on the bound account; token exhaustion rotates the upstream connection without closing the persistent local client when the frame is replayable.
